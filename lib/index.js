...
function schemaValidator(defaultsFile, defaultsObj, cfg, entity, isFile) {
    ...
    for (var key in defaultsObj) {
        if (!cfg.hasOwnProperty(key)) errs.push(key + "': Invalid option.");
    }
...
module.exports = {
    ...
    getConfig: function(options) {
        ...
        if (options.clone && !fs.existsSync(configFilename)) {
        ...
        validator = function(config, entity, isFile) {
        ...
        validator(homeCfg, homeFile, true);
        ...
        if (options.ancestors) {
            ...
            ancestors.forEach(function(ancestorFile) {
                ...
                validator(homeCfg, ancestorFile, true);
        ...
        if (options.env && typeof options.env === "string") {
            ...
            validator('$' + options.env, cli_cfg, false);
        ...
        if (cli_cfg) {
            validator('command line args', cli_cfg, false);
            cfg = fnMerge(cfg, cli_cfg);
        }

        // Merge application overrides:
        if (options.override) {
            validator('options.override', options.override, false);
