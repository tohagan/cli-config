...
module.exports = {
     ...
     *     - `[parser]`         {Function} .config parser. So you can parse YAML or Coffee script or use an improve JSON parser (e.g. might reports errors better).  (default: JSON.parser). Throws exceptions on parse failure.
     *
    ...
    getConfig: function(options) {
        ...
        var defaultsFile = options.dirname ? path.join(options.dirname, configFilename) : null;

        var parser = options.parser || JSON.parse;  // BYO parser

        // Clone defaultsFile to user's home config file on first use
        if (defaultsFile && options.clone && !fs.existsSync(configFilename)) {
            // Not using streams - avoids async issues on Windows
            fs.writeFileSync(configFilename, fs.readFileSync(defaultsFile, 'utf8'), 'utf8');
        ...
        var cfg = readConfigFile(parser, defaultsFile) || {};

        // Add 'package.json' as ._pkg field
        cfg._pkg = pkg;

        // Merge config file in home directory or options.configFile:
        var homeConfig = expandHomeDir('~/' + configFilename);
        cfg = fnMerge(cfg, readConfigFile(parser, homeConfig) || {});
